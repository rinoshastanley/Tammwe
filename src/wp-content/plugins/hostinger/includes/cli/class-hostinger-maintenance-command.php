...
class Hostinger_Maintenance_Command {
	...
	public function mode( array $args ): void {
		...
		switch ( $args[0] ) {
			case '1':
				update_option( 'hostinger_maintenance_mode', 1 );
				WP_CLI::success( 'Maintenance mode ENABLED' );
				break;
			case '0':
				update_option( 'hostinger_maintenance_mode', 0 );
	...
	public function status(): bool {
		$status = get_option( 'hostinger_maintenance_mode' );
		WP_CLI::success( 'Maintenance mode status: ' . $status );
